...
func Add(mgr manager.Manager) error {
	...
	svr, err := webhook.NewServer("machine-admission-server", mgr, webhook.ServerOptions{
		Port:    9876,
		CertDir: "/tmp/cert",
		BootstrapOptions: &webhook.BootstrapOptions{
			...
			Service: &webhook.Service{
				Namespace: ns,
				Name:      "machine-webhook-service",
				// Selectors should select the pods that runs this webhook server.
				Selectors: map[string]string{
					"cluster.k8s.io/validatingwebhook": v1alpha1.GroupName,
				},
			},
		},
	})
	...
	svr.ValidatingWebhookConfigName = "cluster-api-provider-openstack-validating-machine"
